...
class FileListViewer(QWidget):
    ...
    def __init_ui(self):
        ...
        # Save 버튼 생성
        self.save_button = create_tree_view_button("save_json_data_button_icon.svg", 20, 20, self.__test_func)
        
        # Refresh 버튼 생성
        self.refresh_button = create_tree_view_button("refresh_json_data_button_icon.svg", 20, 20, self.__test_func)

        # Rename 버튼 생성
        self.rename_button = create_tree_view_button("rename_json_data_button_icon.svg", 20, 20, self.__test_func)
        
        # + 버튼 생성 (파일 추가)
        self.add_button = create_tree_view_button("plus_button_icon.svg", 20, 20, self.add_file)

        # - 버튼 생성 (파일 삭제)
        self.remove_button = create_tree_view_button("minus_button_icon.svg", 20, 20, self.__test_func)
    ...
    def __update_list(self):
        ...
        # unsaved file flag 없음
        self.file_unsaved_flags = [False] * len(file_names)

        # TreeView에 item 추가
        self.tree_view.add_items_without_selected_flags(file_names, self.file_unsaved_flags)
        ...
        if self.target_file not in file_names:
